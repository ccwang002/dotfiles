...
function __liang_pretty_workdir -d 'Print shortened work_dir'
...
function __bobthefish_project_pwd -d 'Print the working directory relative to project root'
  ...
  set -l work_dir (echo $PWD | sed -e "s*$base_dir**g" -e 's*^/**')
  echo -n (__liang_pretty_workdir $work_dir)
end

function __liang_ipaddr_last_digit -d 'Print last digit of IP address'
  echo -n -s (ifconfig | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | cut -d. -f4 | head -n 1)
end
...
function __liang_prompt_vf_status -d 'Display status of virtualfish'
  if set -q VIRTUAL_ENV
      __bobthefish_start_segment $dk_blue $lt_blue
